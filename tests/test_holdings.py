...
class TestSummarizeHoldings:
    ...
    def test_allocation_groups_by_asset_class(self):
        result = summarize_holdings(
            [
                holding(value=1000.0, assetClass="US Stocks"),
                holding(value=500.0, assetClass="US Stocks"),
                holding(value=500.0, assetClass="Bonds"),
            ]
        )
        assert result["allocation"]["US Stocks"]["value"] == 1500.0
        assert result["allocation"]["Bonds"]["value"] == 500.0

    def test_percentages_are_shares_of_the_total(self):
        result = summarize_holdings(
            [holding(value=750.0, assetClass="US Stocks"), holding(value=250.0, assetClass="Bonds")]
        )
        assert result["allocation"]["US Stocks"]["pct"] == 75.0
        assert result["allocation"]["Bonds"]["pct"] == 25.0

    def test_allocation_is_sorted_by_value_descending(self):
        result = summarize_holdings(
            [
                holding(value=100.0, assetClass="Cash"),
                holding(value=900.0, assetClass="US Stocks"),
                holding(value=400.0, assetClass="Bonds"),
            ]
        )
        assert list(result["allocation"]) == ["US Stocks", "Bonds", "Cash"]

    def test_zero_total_does_not_divide_by_zero(self):
        result = summarize_holdings([holding(value=0.0)])
        assert result["total_value"] == 0.0
        assert result["allocation"]["US Stocks"]["pct"] == 0

    def test_missing_or_null_asset_class_becomes_unknown(self):
        result = summarize_holdings(
            [holding(assetClass=None), {"value": 5.0, "accountName": "Brokerage"}]
        )
        assert "Unknown" in result["allocation"]
        assert result["allocation"]["Unknown"]["value"] == 1005.0
    ...
    def test_by_account_groups_and_keeps_holding_fields(self):
        result = summarize_holdings(
            [
                ...
                holding(ticker="BND", accountName="Brokerage", value=500.0, assetClass="Bonds"),
        ...
        entry = result["by_account"]["401k"][0]
        assert entry == {
            "ticker": "VXUS",
    ...
    def test_negative_values_are_preserved(self):
        ...
        result = summarize_holdings(
            [holding(value=1000.0, assetClass="US Stocks"), holding(value=-200.0, assetClass="Cash")]
        )
        assert result["total_value"] == 800.0
        assert result["allocation"]["Cash"]["value"] == -200.0
...
class TestAssetAllocationTool:
    ...
    def test_allocation_rows_show_value_and_percentage(self, install_api):
        install_api(
            holdings=[
                holding(value=750.0, assetClass="US Stocks"),
                holding(value=250.0, assetClass="Bonds"),
            ]
        )
        text = get_asset_allocation()
        assert re.search(r"US Stocks\s+\$\s*750\.00\s+75\.0%", text)
        assert re.search(r"Bonds\s+\$\s*250\.00\s+25\.0%", text)

    def test_bar_length_is_half_the_percentage(self, install_api):
        install_api(holdings=[holding(value=1000.0, assetClass="US Stocks")])
        line = [ln for ln in get_asset_allocation().splitlines() if "US Stocks" in ln][0]
    ...
    def test_only_the_top_25_holdings_per_account_are_listed(self, install_api):
        install_api(
            holdings=[
                holding(ticker=f"T{i:02d}", accountName="Brokerage", value=float(100 - i))
                for i in range(30)
            ]
        )
        text = get_asset_allocation()
